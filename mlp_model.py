...
def build_model():
    d = 0.3
    model = Sequential()
    model.add(Dense(units = 256 , input_shape=(20,5) , kernel_initializer="uniform"))
    model.add(Dropout(d))
    model.add(Dense(units = 16 , input_shape=(20,5) , kernel_initializer="uniform"))
    model.add(Dropout(d))
    model.add(Flatten())
